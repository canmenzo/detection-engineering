...
- **19 Sigma rules**, each ATT&CK-mapped and scored against hand-labelled benign
  look-alikes: 15 Windows rules also proven to fire on a pinned public EVTX
...
19 rules, 158 labelled events. Generated by `detkit eval`; CI fails if this table drifts from the corpus.

| rule | level | precision | recall | FP rate | bar it must clear |
|---|---|---:|---:|---:|---|
| `entra_audit_privileged_role_assigned` | high | 1.00 | 1.00 | 0.00 | p≥0.90, r≥1.00, fp≤0.10 |
...
| `entra_signin_legacy_auth_success` | medium | 0.80 | 1.00 | 0.20 | p≥0.80, r≥1.00, fp≤0.20 |
| `sysmon_proc_access_lsass_dump` | high | 0.80 | 1.00 | 0.20 | p≥0.75, r≥1.00, fp≤0.25 |
| `entra_audit_illicit_consent_grant` | high | 0.80 | 1.00 | 0.25 | p≥0.75, r≥1.00, fp≤0.25 |
...
| `entra_audit_app_credential_added` | high | 0.60 | 1.00 | 0.40 | p≥0.55, r≥1.00, fp≤0.40 |
